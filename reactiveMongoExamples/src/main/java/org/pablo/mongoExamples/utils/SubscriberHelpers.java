...
public final class SubscriberHelpers {
	...
	/**
	 * A Subscriber that stores the publishers results and provides a latch so
	 * can block on completion.
	 *
	 * @param <T>
	 *            The publishers result type
	 */
	public static class ObservableSubscriber<T> implements Subscriber<T> {
		...
		public ObservableSubscriber<T> await(final long timeout,
				final TimeUnit unit) throws Throwable {
			subscription.request(Integer.MAX_VALUE);
			if (!latch.await(timeout, unit)) {
				throw new MongoException("Publisher onComplete timed out");
			}
			if (!errors.isEmpty()) {
				throw errors.get(0);
			}
	...
	/**
	 * A Subscriber that prints a message including the received items on
	 * completion
	 *
	 * @param <T>
	 *            The publishers result type
	...
	public static class PrintSubscriber<T> extends OperationSubscriber<T> {
		 ...
		 *
		 * @param message
		 *            the message to output onComplete
		 */
	 ...
	 */
	public static class PrintDocumentSubscriber extends
			OperationSubscriber<Document> {
