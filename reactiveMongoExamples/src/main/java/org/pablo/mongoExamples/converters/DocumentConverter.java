...
public interface DocumentConverter {
	...
	static Person apply(Document document) {
		return new Person(document.get(PersonFields.FIRST_NAME.getCode())
				.toString(), document.get(PersonFields.LAST_NAME.getCode())
				.toString());
	}

	static Document apply(Person person) {
		return new Document().append(PersonFields.FIRST_NAME.getCode(),
				person.getFirstname()).append(PersonFields.LAST_NAME.getCode(),
				person.getLastname());
	}
