...
public class ReactivePersonRepositoryImpl extends ReactiveAbstractMongoDao implements PersonRepository{
	...
	public List<Person> findAllPersonList() throws MongoException {
		return super.findAll().stream().map(document -> DocumentConverter.apply(document)).collect(Collectors.toList());
	}

	@Override
	public Stream<Person> findAllPersonStreams() throws MongoException {
		return super.findWithStream().map(document -> DocumentConverter.apply(document));
	}
